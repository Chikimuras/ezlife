...
import type { WeeklyInsight } from '@/lib/api/schemas/insights'
...
const formatWeekRange = (start: string, end: string) => {
  ...
  const formatter = new Intl.DateTimeFormat(t('common.locale'), {
    day: 'numeric',
    month: 'short',
  })
...
<template>
  <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
    <div class="flex items-center justify-between mb-4">
      <div>
        <h2 class="text-lg font-semibold text-gray-900">
          {{ t('dashboard.totalTime.titleWeekly') }}
        </h2>
        <p class="text-xs text-gray-500 mt-1">
          {{ formatWeekRange(insight.weekStartDate, insight.weekEndDate) }}
        </p>
      </div>
    ...
    <div class="mb-4">
      ...
      <p class="text-sm text-gray-600 mt-1">{{ t('dashboard.totalTime.thisWeek') }}</p>
    </div>
    ...
    <div class="flex items-center gap-2 pt-4 border-t border-gray-100">
      ...
      <span class="text-sm text-gray-600">
        ...
        {{ t('dashboard.totalTime.vsLastWeek') }}
      </span>
    </div>

    <div class="mt-3 text-xs text-gray-500">
      {{ t('dashboard.totalTime.lastWeek') }}: {{ previousHours }}h {{ previousMinutesRemainder }}m
    </div>
  </div>
</template>
